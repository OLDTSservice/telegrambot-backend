...
def _call_claude(question: str, chunks: list[str], bot_id: int) -> Optional[tuple]:
    ...
    try:
        ...
        system_prompt = (
            ...
            "不要加入任何額外補充、說明或開場白。"
            "重要規則：\n"
            "1. 只有在知識庫中有【直接對應的答案】時才回答，不可自行推斷、整合或延伸事實內容"
            "（不可新增知識庫沒有的資訊、不可補充原本沒有的細節）。\n"
            "1a. 【務必逐一檢查清單類內容】知識庫裡常有清單或表格（例如遊戲列表、支援項目清單），"
            "使用者可能會針對清單中「某一個特定項目」提問（例如「XXX 遊戲有沒有支援 YYY？」）。"
            "遇到這類問題時，你必須先在知識庫的每一份清單內容裡，逐項搜尋是否包含該項目名稱："
            "只要在任一清單找到該項目，就直接依清單內容回答「是/否/該清單所列的狀態」，這屬於【直接引用】，"
            "不算違反規則 1 的「不可推斷延伸」；只有確認該項目完全不在任何清單中才視為無答案。"
            "範例：知識庫裡有一段列出「支援 Buy Bonus 的遊戲」清單，其中一行是「523 Fortune Coins」，"
            "使用者問「Does Fortune Coins support Buy Bonus?」——因為 Fortune Coins 確實出現在該清單中，"
            "應回答「Yes, Fortune Coins supports Buy Bonus.」；不可僅因為知識庫沒有『用一整句話明確寫出是否支援』"
            "就判定為無答案，那樣過於保守，清單裡的存在本身就是答案。\n"
            ...
            f"{translate_note}"
            "3b. 知識庫內容有時會把同一份資源（例如同一份文件、連結）的不同語言版本綁在同一段答案裡"
            "（例如中文版連結後面接著一段英文說明與英文版連結）。回覆時只保留符合規則 3 判定之目標語言的"
            "那個版本，其他語言版本的文字與連結一律整段刪除、不要一併附上；除非使用者在問題中明確要求"
            "多個語言版本或完整清單。這屬於【篩選】而非「自行推斷延伸」，不算違反規則 1。\n"
