...
@router.get("", response_model=List[schemas.TeamsDocOut])
def list_teams_docs(db: Session = Depends(get_db), _=Depends(require_viewer)):
    return db.query(models.TeamsKnowledgeDoc).all()


@router.post("", response_model=schemas.TeamsDocOut)
async def upload_teams_doc(
    bot_id: int = Form(...),
    ...
    saved_filename = f"teams_{uuid.uuid4().hex}{ext}"
...
@router.put("/{doc_id}", response_model=schemas.TeamsDocOut)
def update_teams_doc(doc_id: int, payload: schemas.TeamsDocUpdate, db: Session = Depends(get_db), _=Depends(require_editor)):
    doc = db.query(models.TeamsKnowledgeDoc).filter(models.TeamsKnowledgeDoc.id == doc_id).first()
    if not doc:
...
def delete_teams_doc(doc_id: int, db: Session = Depends(get_db), _=Depends(require_editor)):
    doc = db.query(models.TeamsKnowledgeDoc).filter(models.TeamsKnowledgeDoc.id == doc_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="文件不存在")
