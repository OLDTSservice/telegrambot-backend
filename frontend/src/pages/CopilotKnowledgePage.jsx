...
export default function CopilotKnowledgePage({ user }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [dRes, bRes] = await Promise.all([getCopilotDocs(), getCopilotBots()])
      setDocs(dRes.data); setBots(bRes.data)
  ...
  const handleUpload = async () => {
    if (!selectedBotId) { message.warning('請先選擇機器人'); return }
    if (!pendingFile) { message.warning('請選擇文件'); return }
    const formData = new FormData()
    formData.append('bot_id', selectedBotId)
    formData.append('file', pendingFile)
    setUploading(true)
    try {
      await uploadCopilotDoc(formData)
      message.success('文件上傳並處理完成')
      setUploadModalOpen(false); setPendingFile(null); setSelectedBotId(null); load()
    } catch (err) { message.error(err.response?.data?.detail || '上傳失敗') }
    finally { setUploading(false) }
  }

  const columns = [
    { title: '啟用', dataIndex: 'is_enabled', width: 70, render: (v, r) => <Switch checked={v} size="small" onChange={c => updateCopilotDoc(r.id, { is_enabled: c }).then(load)} disabled={!canEdit(user)} /> },
    {
      title: '文件名稱', dataIndex: 'original_filename', render: (name, r) => (
        <Space>{FILE_ICON[r.file_type] || <FileTextOutlined />}<Text>{name}</Text></Space>
      )
    },
  ...
  return (
    <Card>
      <div className="page-header">
        <h2>Copilot 知識庫管理</h2>
        {canEdit(user) && <Button type="primary" icon={<UploadOutlined />} onClick={() => setUploadModalOpen(true)}>上傳文件</Button>}
      </div>
      <Text type="secondary" style={{ display: 'block', marginBottom: 16, fontSize: 13 }}>
        上傳文件後，Copilot 機器人將根據文件內容以 AI 回覆相關問題。支援 PDF、Word、Excel、TXT、CSV、MD。
      </Text>
      <Table rowKey="id" dataSource={docs} columns={columns} loading={loading} pagination={{ pageSize: 10 }} locale={{ emptyText: '尚未上傳任何知識庫文件' }} />

      <Modal title="上傳知識庫文件" open={uploadModalOpen}
        onOk={handleUpload} onCancel={() => { setUploadModalOpen(false); setPendingFile(null); setSelectedBotId(null) }}
        okText="上傳並處理" cancelText="取消" confirmLoading={uploading}>
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <Text strong>綁定機器人</Text>
            <Select style={{ width: '100%', marginTop: 6 }} placeholder="選擇機器人" value={selectedBotId} onChange={setSelectedBotId}>
              {bots.map(b => <Select.Option key={b.id} value={b.id}>{b.name}</Select.Option>)}
            </Select>
          </div>
          <div>
            <Text strong>選擇文件</Text>
            <Upload beforeUpload={f => { setPendingFile(f); return false }} maxCount={1}
