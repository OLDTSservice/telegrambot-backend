...
import {
  Table, Button, Switch, Upload, Select, Modal,
...
export default function TeamsKnowledgePage({ user }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [dRes, bRes] = await Promise.all([getTeamsDocs(), getTeamsBots()])
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try {
      await uploadTeamsDoc(formData)
      message.success('文件上傳並處理完成')
      setUploadModalOpen(false); setPendingFile(null); setSelectedBotId(null)
      load()
  ...
  const handleToggle = async (doc, checked) => {
    try { await updateTeamsDoc(doc.id, { is_enabled: checked }); load() }
    catch { message.error('切換失敗') }
  }

  const handleDelete = async id => {
    try { await deleteTeamsDoc(id); message.success('已刪除'); load() }
  ...
  const columns = [
    ...
    {
      title: '文件名稱', dataIndex: 'original_filename',
      render: (name, record) => (
        <Space>{FILE_ICON[record.file_type] || <FileTextOutlined />}<Text>{name}</Text></Space>
      ),
    },
    ...
    { title: '上傳時間', dataIndex: 'created_at', width: 160, render: t => new Date(t).toLocaleString('zh-TW') },
    {
      title: '操作', width: 80,
      render: (_, record) => (
        <Popconfirm title="確定刪除此文件？" onConfirm={() => handleDelete(record.id)} disabled={!canEdit(user)}>
  ...
  return (
    <Card>
      <div className="page-header">
        <h2>Teams 知識庫管理</h2>
        {canEdit(user) && (
          <Button type="primary" icon={<UploadOutlined />} onClick={() => setUploadModalOpen(true)}>上傳文件</Button>
        )}
      </div>
      <Text type="secondary" style={{ display: 'block', marginBottom: 16, fontSize: 13 }}>
        上傳文件後，Teams 機器人將根據文件內容以 AI 回覆相關問題。支援格式：PDF、Word、Excel、TXT、CSV
      </Text>
      <Table rowKey="id" dataSource={docs} columns={columns} loading={loading}
        pagination={{ pageSize: 10 }} locale={{ emptyText: '尚未上傳任何知識庫文件' }} />

      <Modal
        title="上傳 Teams 知識庫文件"
      ...
      >
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <Select style={{ width: '100%', marginTop: 6 }} placeholder="選擇 Teams 機器人"
              value={selectedBotId} onChange={setSelectedBotId}>
